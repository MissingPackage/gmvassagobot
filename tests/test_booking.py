...
def test_è_slot_valido():
    assert è_slot_valido(datetime(2025, 12, 31, 12, 0)) is True
    assert è_slot_valido(datetime(2025, 12, 31, 23, 59)) is False
    assert è_slot_valido(datetime(2025, 12, 31, 11, 59)) is False
    assert è_slot_valido(datetime(2025, 12, 31, 15, 0)) is True
...
def test_estrai_data_ora():
    assert estrai_data_ora("31 dicembre 2025 alle 16:00") == datetime(2025, 12, 31, 16, 0)
...
def test_gestisci_prenotazione(mocker):
    mocker.patch("app.booking.estrai_data_ora", return_value=datetime(2025, 12, 31, 16, 0))
    ...
    result = gestisci_prenotazione("Prenota per il 31 dicembre 2025 alle 16:00 per 30 minuti")
...
def test_gestisci_prenotazione_slot_non_valido(mocker):
    mocker.patch("app.booking.estrai_data_ora", return_value=datetime(2025, 12, 31, 11, 0))
    result = gestisci_prenotazione("Prenota per il 31 dicembre 2025 alle 11:00")
...
def test_suggerisci_slot_alternativi(mocker):
    ...
    start_time = datetime(2025, 12, 31, 16, 0)
